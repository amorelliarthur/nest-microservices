...
describe('UsersService', () => {
    ...
    beforeEach(async () => {
        ...
        userModel = jest.fn().mockImplementation((data: any) => ({
            ...data,
            save: jest.fn().mockResolvedValue({ ...data, _id: '6839a1b2c3d4e5f6a7b8c9d0' }),
        }));
    ...
    describe('create()', () => {
        ...
        it('deve criar um novo usuário quando não existe email ou cpf cadastrado', async () => {
            ...
            expect(userModel).toHaveBeenCalledWith(
                expect.objectContaining({ email: dto.email, senha: 'hashed-senha', deletedAt: null }),
            );
        ...
        it('deve reativar o usuário quando ele existe mas está com deletedAt preenchido', async () => {
            ...
            expect(rabbitMQService.publish).toHaveBeenCalledWith('user.created', expect.any(Object));
            expect(result).toBeDefined();
        });
    });
    ...
    describe('findById()', () => {
        ...
        it('deve buscar no MongoDB e popular o cache quando houver MISS no Redis', async () => {
            ...
            expect(userModel.findOne).toHaveBeenCalledWith({ _id: userId, deletedAt: null });
    ...
    describe('authenticate()', () => {
        ...
        it('deve lançar NotFoundException quando o usuário não existe', async () => {
            ...
            await expect(service.authenticate('inexistente@user.br', 'Senha@123')).rejects.toThrow(
                NotFoundException,
            );
        });
        ...
        it('deve lançar UnauthorizedException quando a senha está incorreta', async () => {
            ...
            await expect(service.authenticate('arthur@user.br', 'errada')).rejects.toThrow(
    ...
    describe('update()', () => {
        ...
        it('deve atualizar o usuário e invalidar o cache', async () => {
            const userAtualizado = buildUserDoc({ _id: userId, nome: 'Arthur Atualizado' });
            userModel.findOneAndUpdate.mockResolvedValue(userAtualizado);

            const result = await service.update(userId, { nome: 'Arthur Atualizado' } as any);
        ...
        it('deve lançar NotFoundException quando o usuário não existe', async () => {
            ...
            await expect(service.update(userId, { nome: 'X' } as any)).rejects.toThrow(
                NotFoundException,
            );
